# Synthesis Contract — Nike Chill Run Club (Stage 7.1, re-run)

Framework: `7.1 Synthesis.md` (current — Camera Specs + Luminance Hierarchy verbatim pass-through)
Inputs: Campaign (1.1), Brand (0), Strategy (2), Narrative (3), Art Direction (4), Scene (5.2.5), Composition & Rendering (6.1, re-run)
...
This is a re-run of Trial_07's Stage 7.1 against the updated framework and the re-run 06 contract. All communication, hierarchy, execution, and preservation resolutions are unchanged from Trial_07 — only `PhysicalRendering.CameraSpecs.aperture_f_stop`, the new `PhysicalRendering.LuminanceHierarchy`, and the rendering-quality/optical prose tied to depth and lighting differ.
  ...
  "SynthesisContract": {
    ...
    "RenderingResolution": {
      ...
      "PhysicalRendering": {
        ...
        "OpticalIntent": "Atmospheric/Soft Focus falloff consistent with CameraSpecs (35mm, f/5) — group, Pegasus, railing, and near water sharp to clearly legible; mid-distance towers soften gradually and the far skyline reads as soft, recognizable light shapes rather than heavy circular bokeh, with skyline brightness diffused rather than blown-out; no flat cutout blur"
      },

      "RenderingQuality": {
        "DetailRendering": "Faces, expressions, and the Pegasus identifiers sharp and legible; skyline softened by depth but recognizable as Hong Kong",
        "TextureRendering": "Visible skin pores and fabric weave; tactile shoe and pavement surfaces",
        "LightingRendering": "Warm key on the group reading brightest, cooler ambient fill on promenade/shoes held visibly dimmer, skyline window-lights diffused and dimmest",
        "DepthRendering": "Clear foreground/midground/background separation via Atmospheric falloff and haze, with the skyline kept contextually present",
...
- **CameraSpecs** (35mm, f/5) carried verbatim from the re-run `CameraPlan` per the 6.1 → 7.1 pass-through rule — replaces Trial_07's f/2.8; `CameraIntent` and `OpticalIntent` reference but do not restate the numbers.
...
- All communication, hierarchy, execution, and preservation resolutions unchanged from Trial_07 — only camera specs, luminance hierarchy, and the directly dependent rendering-quality/optical prose differ.
