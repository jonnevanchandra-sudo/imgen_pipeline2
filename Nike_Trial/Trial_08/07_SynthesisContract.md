...
Inputs: CampaignContract (1.1) + BrandContract (0) + StrategyContract (2) + NarrativeContract (3) + ArtDirectionContract (4) + SceneContract (5.2.5) + CompositionRenderingContract (6.1)
  ...
  "SynthesisContract": {
    ...
    "RenderingResolution": {
      ...
      "PerceptualRendering": {
        ...
        "Atmosphere": "A vibrant Hong Kong night: warm ambient light around the group against a cooler, glowing cross-harbour skyline."
      },
      ...
      "PhysicalRendering": {
        ...
        "LightingIntent": "Warm practical lighting (promenade lamps, ambient city glow) wraps the group as the brightest, warmest tones in the frame; the cross-harbour skyline reads cooler and dimmer, glowing but never competing with the group.",
        "LuminanceHierarchy": [
      ...
      "RenderingQuality": {
        ...
        "LightingRendering": "Warm color temperature on the group from practical lamps; cooler tones on the distant skyline; soft directional shadows.",
...
- ✓ `CameraSpecs` (`35mm`, `f/4`) carried verbatim from CompositionRenderingContract's `CameraPlan`, kept as structured values separate from `CameraIntent` prose.
- ✓ `LuminanceHierarchy` carried verbatim per entity from `HierarchyPlan`; `LightingIntent` describes mechanism without restating the ranking as a list.
- ✓ Preservation Resolution protects ProductSpec details and named-product identity through to the Prompt Compiler's `brand` key (HIGH).
