...
```json
{
  "CompositionRenderingContract": {
    ...
    "HierarchyPlan": [
      {
        ...
        "reason": "The group's energized 'mode switch' is the campaign's core communication and must dominate both attention and exposure.",
        "source": "Narrative"
      },
      {
        "element": "entity_02",
      ...
      },
      {
        ...
        "reason": "The skyline establishes 'Hong Kong at night' but must never compete with or outshine the group.",
        "source": "Environmental"
      }
    ],
    ...
    "AttentionPlan": {
      ...
      "reason": "Luminance and sharpness keep the group brightest and crispest against a softer environment; the group's own movement direction (gaze/body vectors) leads the eye toward the shoes and then out across the plaza to the skyline, giving a single readable path without competing focal points."
    },
    ...
    "LightingPlan": {
      ...
      "possible_techniques": ["rim_lighting_emphasis", "warm_practical_lighting"],
      "reason": "Warm promenade lamp light and ambient city glow wrap the group, keeping them the brightest, warmest tones in the frame (Brightest priority). The skyline across the harbour is rendered cooler and dimmer by comparison, preventing it from competing with the group per the Dimmest ranking, while still reading as a lit, lively night skyline."
    },
...
- ✓ Hierarchy readable: group (Primary/Brightest) → Pegasus & plaza (Secondary/Mid) → skyline (Supporting/Dimmest); no Secondary/Supporting element outranks Primary.
- ✓ Depth of Field Category: **Atmospheric** (default), `f/4` falls within `f/4`–`f/5.6`; selected because the plaza/skyline carry contextual story (Medium structural density) but `AttentionPlan` does not call for full isolation.
...
- ✓ `pose_variation_required: true` — multiple subjects share a running/transitioning action; Composition & Rendering flags independent pose/gait phases for downstream Synthesis/Prompt translation.
